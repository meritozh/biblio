...
#[tauri::command]
pub async fn prompt_list(app: tauri::AppHandle) -> Result<Vec<Prompt>, String> {
    let instances = app.state::<DbInstances>();
    let pool = get_sqlite_pool(&instances, "sqlite:biblio.db")?;

    let prompts: Vec<Prompt> = sqlx::query_as(
        "SELECT id, name, content, is_default, created_at, updated_at FROM prompts ORDER BY created_at DESC",
    )
    .fetch_all(&pool)
    .await
...
pub async fn prompt_create(
    ...
    sqlx::query(
        "INSERT INTO prompts (name, content, is_default) VALUES (?, ?, 0)",
    )
    .bind(&payload.name)
    ...
    let prompt: Prompt = sqlx::query_as(
        "SELECT id, name, content, is_default, created_at, updated_at FROM prompts WHERE id = last_insert_rowid()",
    )
    .fetch_one(&pool)
    .await
    .map_err(|e| e.to_string())?;
...
pub async fn prompt_update(
    ...
    sqlx::query(
        "UPDATE prompts SET name = ?, content = ?, updated_at = datetime('now') WHERE id = ?",
    )
    .bind(&payload.name)
    .bind(&payload.content)
    .bind(id)
    ...
    let prompt: Prompt = sqlx::query_as(
        "SELECT id, name, content, is_default, created_at, updated_at FROM prompts WHERE id = ?",
    )
    .bind(id)
    .fetch_one(&pool)
    .await
